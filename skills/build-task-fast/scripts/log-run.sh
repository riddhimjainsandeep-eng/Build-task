...
#
# Called from Phase 5(e), via close-out.sh. NEVER blocks the run: every failure
# path warns on stderr and exits 0, because a missing history row is not worth
# failing a completed piece of work over.
#
...
# The table is append-only, so a duplicate can never be removed. Phase 5 is
# meant to be safe to repeat: skip if this repo/task/date is already logged. If
# the check itself fails, fall through to the write, as before.
enc() { jq -rn --arg v "$1" '$v|@uri'; }
EXISTING=$(curl -sS --max-time 20 \
  "$URL/rest/v1/build_runs?select=task&limit=1&repo=eq.$(enc "$REPO")&task=eq.$(enc "$TASK")&run_date=eq.$RUN_DATE" \
  -H "apikey: $KEY" -H "Authorization: Bearer $KEY" 2>/dev/null)
FOUND=$(jq -r 'if type=="array" then length else 0 end' <<<"$EXISTING" 2>/dev/null | tr -d '\r')
if [ "${FOUND:-0}" -gt 0 ] 2>/dev/null; then
  echo "already in the shared history ($REPO / $TASK / $RUN_DATE) — not logging twice"
  exit 0
fi
